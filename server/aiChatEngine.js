...
class AIChatEngine {
  ...
  async generateResponse(message) {
    ...
    if (this.conversationState.stage === "initial") {
      ...
      this.conversationState.category = category.category;
      this.conversationState.confidence = category.confidence;
      this.conversationState.stage = "asking_child_name";
      return {
        text: `I understand you're asking about ${category.category}. To help you better, could you tell me which child you're concerned about?`,
        category: "Who",
      };
      ...
      return {
        text: `Thank you. What specific ${this.conversationState.category.category} behavior did ${this.conversationState.childName} exhibit?`,
        category: "What",
      };
  ...
  provideDetailedAdvice() {
    ...
    switch (this.conversationState.category) {
      case "stealing":
        advice = this.getDetailedStealingAdvice();
        break;
      case "greeting":
        advice = this.getDetailedGreetingAdvice();
        break;
      case "lying":
        advice = this.getDetailedLyingAdvice();
        break;
      case "meltdowns":
        advice = this.getDetailedMeltdownsAdvice();
        break;
      case "not_listening":
        advice = this.getDetailedNotListeningAdvice();
        break;
      case "temper_tantrums":
        advice = this.getDetailedTemperTantrumsAdvice();
        break;
      default:
        advice =
          "I'm sorry, I don't have specific advice for this situation. However, it's important to approach the issue with understanding and open communication with your child.";
    }

    return {
      text: `I understand you're concerned about ${this.conversationState.childName}'s ${this.conversationState.category} behavior. 
      Here's some advice that might help:
      
      ${advice}
      
      Was this recommendation helpful? If so, give me a thumbs up. If not, give me a thumbs down. If you don't mind, take a few minutes to explain why, and I can try to provide a better response.`,
  ...
  getDetailedLyingAdvice() {
    return `I understand you're concerned about ${this.conversationState.childName}'s lying. Here's some advice that might help:
  ...
  getDetailedMeltdownsAdvice() {
    return `I'm sorry to hear that ${this.conversationState.childName} is experiencing meltdowns. Here's some advice to help manage these situations:
  ...
  getDetailedNotListeningAdvice() {
    return `I understand you're having difficulties with ${this.conversationState.childName} not listening. Here are some strategies that might help:
  ...
  getDetailedTemperTantrumsAdvice() {
    return `I'm sorry to hear that ${this.conversationState.childName} is having temper tantrums. Here's some advice to help manage these challenging situations:
  ...
  getDetailedStealingAdvice() {
    return `1. Have a calm, non-judgmental conversation with ${this.conversationState.childName} about the incident. Try to understand their motivations without accusation.
