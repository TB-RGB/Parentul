...
async function logChatHistory(userId, userMessage, aiResponse) {
  ...
  try {
    ...
    LIMIT 1;
    `;
    const conversationCheck = await client.query(checkConversationQuery, [
      userId,
    ]);
    let conversationId;
    ...
    if (conversationCheck.rows.length === 0) {
      ...
      const newConversationQuery = `
            ...
            RETURNING id;
            `;
      const newConversation = await client.query(newConversationQuery, [
        userId,
      ]);
